classdef LiveFeedManager
   ...
   methods (Static)
       ...
       function f = start_panel_in_background(data_function,npixel,nsamples)
            fig = figure;
            line = plot(1,1);
            q = parallel.pool.DataQueue;
            afterEach(q,@(data) LiveFeedManager.update_line_plot(line,data));
            function annalyzer = start_live_data_feed(q)
                annalyzer = LiveRadonAnnalyzer(data_function,npixel,nsamples,q);
            ...
            addlistener(fig,'ObjectBeingDestroyed',@(src,eventData)f.cancel);
       end
