       ...
       function results = get_result_from_tif_and_stripes(stripe_statistics,mat,offset_seconds,window_size_seconds,resample_frequency,file_name)
            ...
            for stimulusi=1:n_stimulus 
                try
                    ...
                    sign_change = sign(median(result.filtered_speed_pre_stim));
                    result.mean_pre_stim = mean(result.filtered_speed_pre_stim)*sign_change;
                    result.median_pre_stim = median(result.filtered_speed_pre_stim)*sign_change;
...
%                         ylim([0,3])
%                     end
%                     limit = sort([mean(mean_trace)-0.8*std(mean_trace)-0.1,mean(mean_trace)+0.8*std(mean_trace)+0.1]);
                    limit = sort([quantile(mean_trace,0.1)-0.5,quantile(mean_trace,0.9)+0.5]);
                    ylim(limit)
                end
            end
       end
...
%                 end
       ...
       function [standardized,time] = unify_standardized_trace(stimi_result)
            offset = [stimi_result.median_pre_stim];
