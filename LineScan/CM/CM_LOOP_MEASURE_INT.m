function CM_LOOP_MEASURE_INT (Str_ArrayList)
figure (222)
clf
...
A{1,1}.firstIndexThisObject=1;
A{1,1}.lastIndexThisObject=size(A{1,1}.scanData.path,1);
A{1,1}.imageCh=(num2str(A{1,1}.CorrectionChannelString));
A{1,1}.im_Ch_ind=(A{1,1}.im_Correction_Ch_ind);
A{1,1}.analysisType ='intensity';
A{1,1}.SaturationCorrection=0;
A{1,1}.assignName='line_99';
newname=[Str_ArrayName '_LIGHT'];
pathAnalysisHelper_SCANIMAGE (A,newname);
toplot=evalin('base','line_99_ch4_MAX_int');
figure (222)
plot(toplot);hold on
end
...
for counter=1:1:size(names,1)
  ...
  % k=k+ ~isempty (strfind(field_name,'Dark_Mean_int'));
    ...
    if (k>0)
        ...
        field_name=strrep(field_name, 'MAX_int','_DARK');
