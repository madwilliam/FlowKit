function CM_PATH_ANALYSIS_PLOT_FROM_STR_CTR(STRC)
...
for i=1:1:imax
    ...
    if (i==1)
        [~,figure_title]=fileparts(local_info.fullFileNameMpd);
         figure_title=[figure_title '_CTRL']
    ...
    if strcmp(local_info.analysisType,'diameter')
        yvectorname= [local_info.assignName '_' 'ch' num2str(local_info.imageCh) '_diameter_pix'];
        diam_trace=tac.(yvectorname)*2;
%         diam_mean=mean(diam_trace(:));
%         diam_trace=((diam_trace/diam_mean)-1)*1000;
         plot(tac.(time_vector_field),diam_trace,'r')
         hold on
        yvectorname= [local_info.assignName '_' 'ch' num2str(local_info.imageCh) '_midline_vector'];
         plot(tac.(time_vector_field),tac.(yvectorname),'k')
         
        yvectorname= [local_info.assignName '_' 'ch' num2str(local_info.imageCh) '_point2_vector'];
         plot(tac.(time_vector_field),tac.(yvectorname),'b')

        yvectorname= [local_info.assignName '_' 'ch' num2str(local_info.imageCh) '_point1_vector'];
        plot(tac.(time_vector_field),tac.(yvectorname),'b')
    ...
    elseif strcmp(local_info.analysisType,'radon')
        yvectorname= [local_info.assignName '_' 'ch' num2str(local_info.imageCh) '_radon_sep'];
        ylabelStr='Velocity angle and separation';
        plot(tac.(time_vector_field),tac.(yvectorname),'r')
        hold on
        yvectorname= [local_info.assignName '_' 'ch' num2str(local_info.imageCh) '_radon_theta'];
        plot(tac.(time_vector_field),tac.(yvectorname),'k')
        ylabel(ylabelStr);
        hold off
    else
...
for i=1: size (scanData.scanCoords,2)
    ...
    if ~strcmp(scanData.scanCoords(i).scanShape,'pause')
        ...
        text(number_position (1),number_position (2),[text_name],...
            ...
            'FontSize',8,'Color',ColorSet(line_counter,:))
    end
